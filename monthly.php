...
$sql="SELECT count(distinct `receipt_number`) as visits FROM `invoice4477year05to07anon` WHERE month(date)=$month;";
...
$sql="SELECT count(`receipt_number`) as products FROM `invoice4477year05to07anon` WHERE month(date)=$month;";
		$result = mysql_query($sql)
        or die(mysql_error());
		while ($row = mysql_fetch_assoc($result)) {
...
$sql="SELECT sum(price) as revenues FROM `invoice4477year05to07anon` WHERE month(date)=$month;";
		$result = mysql_query($sql)
        or die(mysql_error());
		while ($row = mysql_fetch_assoc($result)) {
...
$sql="SELECT sum(price-cost) as profit FROM `invoice4477year05to07anon` WHERE month(date)=$month;";
		$result = mysql_query($sql)
        or die(mysql_error());
		while ($row = mysql_fetch_assoc($result)) {
...
echo "<table class='table table-bordered'>
		<tr id='divider'>
			<td colspan=2>
				<h4>Overall Data(for past month)</h4>
			</td>
		</tr>";
echo 	"<tr>
			<td class='span4'>
				<div>
					<strong>
								Overall Visits for the month: $visits</br>
								Overall Products Sold for the month: $products</br>
								Overall Revenues for the month: $english_format_revenues</br>
								Overall Profit for the month: $english_format_profit</br>
					</strong>
...
echo "<table class='table table-bordered'>
		<tr id='divider'>
			...
			$sql="select invoice4477year05to07anon.product_id, product_name, sum(quantity) as sold , sum(invoice4477year05to07anon.price) as revenue, sum(invoice4477year05to07anon.price-invoice4477year05to07anon.cost) as profit from invoice4477year05to07anon join Products4477 on (invoice4477year05to07anon.product_id=Products4477.product_id) where month(date)=$month group by product_id order by sold desc limit 5;";
		$result = mysql_query($sql)
        or die(mysql_error());
		while ($row = mysql_fetch_assoc($result)) {
